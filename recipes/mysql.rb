...
ruby_block "copy mysql plist to ~/Library/LaunchAgents" do
  block do
    ...
    system("cp -f #{plist_location} #{destination} && chown #{WS_USER} #{destination} && chmod 0644 #{destination} ")
  end
end
...
# TODO: Fix this, not able to run it twice
execute "load the mysql plist into the mac daemon startup thing" do
  puts ">>>>>>>>>>>"
  puts "launchctl load -w #{WS_HOME}/Library/LaunchAgents/homebrew.mxcl.mysql.plist"
